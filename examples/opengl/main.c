...
static inline void print_matrix(float m[]) {
...
int main(int argc, char **argv) {
  ...
  // If OHMD_IDS_AUTOMATIC_UPDATE is set to 0, ohmd_ctx_update() must be called
  // at least 10 times per second. It is enabled by default.
  ...
  // calculate lens centers (assuming the eye separation is the distance between
  // the lens centers)
  ...
  // assume calibration was for lens view to which ever edge of screen is
  ...
  while (!done) {
    ...
    while (SDL_PollEvent(&event)) {
      if (event.type == SDL_KEYDOWN) {
        ...
        case SDLK_F1: {
          ...
          SDL_SetWindowFullscreen(gl.window,
                                  gl.is_fullscreen ? SDL_WINDOW_FULLSCREEN : 0);
        } break;
        case SDLK_F2: {
        ...
        case SDLK_d:
          ...
          if ((distortion_coeffs[0] != 0.0) || (distortion_coeffs[1] != 0.0) ||
              (distortion_coeffs[2] != 0.0) || (distortion_coeffs[3] != 1.0)) {
            distortion_coeffs[0] = 0.0;
            distortion_coeffs[1] = 0.0;
            distortion_coeffs[2] = 0.0;
            distortion_coeffs[3] = 1.0;
          } else {
            ohmd_device_getf(hmd, OHMD_UNIVERSAL_DISTORTION_K,
                             &(distortion_coeffs[0]));
          }
          break;
        case SDLK_x:
    ...
    if (crosshair_overlay) {
      ...
      draw_crosshairs(0.1, 2 * left_lens_center[0] / viewport_scale[0] - 1.0f,
                      2 * left_lens_center[1] / viewport_scale[1] - 1.0f);
    }
    ...
    if (crosshair_overlay) {
      ...
      glLineWidth(5.0);
      glColor4f(1.0, 0.5, 0.0, 1.0);
      draw_crosshairs(0.1, 2 * right_lens_center[0] / viewport_scale[0] - 1.0f,
                      2 * right_lens_center[1] / viewport_scale[1] - 1.0f);
    }
