...
void print_infof(ohmd_device *hmd, const char *name, int len,
                 ohmd_float_value val) {
  float f[16];
  assert(len <= 16);
...
void print_infoi(ohmd_device *hmd, const char *name, int len,
                 ohmd_int_value val) {
  int iv[16];
  assert(len <= 16);
...
int main(int argc, char **argv) {
  ...
  for (int i = 0; i < num_devices; i++) {
    ...
    const char *device_class_s[] = {"HMD", "Controller", "Generic Tracker",
                                    "Unknown"};
    ...
    printf("  class:   %s\n",
           device_class_s[device_class > OHMD_DEVICE_CLASS_GENERIC_TRACKER
                              ? 4
                              : device_class]);
    printf("  flags:   %02x\n", device_flags);
  ...
  const char *controls_fn_str[] = {
      "generic",     "trigger",   "trigger_click", "squeeze",
      "menu",        "home",      "analog-x",      "analog-y",
      "anlog_press", "button-a",  "button-b",      "button-x",
      "button-y",    "volume-up", "volume-down",   "mic-mute"};
  ...
  for (int i = 0; i < control_count; i++) {
    printf("%s (%s)%s", controls_fn_str[controls_fn[i]],
           controls_type_str[controls_types[i]],
           i == control_count - 1 ? "" : ", ");
  }
