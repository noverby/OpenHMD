...
#define NREAL_ID 0x0486
#define NREAL_HMD 0x573c
...
static void update_device(ohmd_device *device) {
  ...
  while (true) {
    ...
    // It seems that as a exception to regular usb hid design, we have no packet
    // descriptor In this case just treating all data as IMU data
...
static ohmd_device *open_device(ohmd_driver *driver, ohmd_device_desc *desc) {
  ...
  omat4x4f_mult(&priv->base.properties.proj_right, &flip,
                &priv->base.properties.proj_right);
  omat4x4f_mult(&priv->base.properties.proj_left, &flip,
...
static void get_device_list(ohmd_driver *driver, ohmd_device_list *list) {
  struct hid_device_info *devs = hid_enumerate(NREAL_ID, NREAL_HMD);
  struct hid_device_info *cur_dev = devs;

  while (cur_dev) {
    // This is needed because the same chip is used in Nolo and DeePoon E2.
    if (wcscmp(cur_dev->product_string, L"NREAL GLASSES") == 0) {
      ohmd_device_desc *desc = &list->devices[list->num_devices++];

      strcpy(desc->driver, "OpenHMD NReal driver");
      strcpy(desc->vendor, "NReal");
      strcpy(desc->product, "NReal Developer Edition");
