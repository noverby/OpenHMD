...
OHMD_APIENTRYDLL const char *OHMD_APIENTRY
ohmd_ctx_get_error(ohmd_context *ctx) {
...
OHMD_APIENTRYDLL int OHMD_APIENTRY ohmd_gets(ohmd_string_description type,
                                             const char **out) {
  switch (type) {
...
OHMD_APIENTRYDLL const char *OHMD_APIENTRY
ohmd_list_gets(ohmd_context *ctx, int index, ohmd_string_value type) {
...
OHMD_APIENTRYDLL int OHMD_APIENTRY ohmd_list_geti(ohmd_context *ctx, int index,
                                                  ohmd_int_value type,
                                                  int *out) {
  if (index >= ctx->list.num_devices)
...
static unsigned int ohmd_update_thread(void *arg) {
  ...
  while (!ctx->update_request_quit) {
    ...
    for (int i = 0; i < ctx->num_active_devices; i++) {
      if (ctx->active_devices[i]->settings.automatic_update &&
          ctx->active_devices[i]->update)
...
OHMD_APIENTRYDLL ohmd_device *OHMD_APIENTRY ohmd_list_open_device_s(
    ohmd_context *ctx, int index, ohmd_device_settings *settings) {
  ohmd_lock_mutex(ctx->update_mutex);
  ...
  if (index >= 0 && index < ctx->list.num_devices) {
    ...
    if (device == NULL) {
      ohmd_set_error(
          ctx,
          "Could not open device with index: %d, check device permissions?",
          index);
      ohmd_unlock_mutex(ctx->update_mutex);
      return NULL;
    }
...
OHMD_APIENTRYDLL ohmd_device *OHMD_APIENTRY
ohmd_list_open_device(ohmd_context *ctx, int index) {
...
static int ohmd_device_getf_unp(ohmd_device *device, ohmd_float_value type,
                                float *out) {
  switch (type) {
  case OHMD_LEFT_EYE_GL_MODELVIEW_MATRIX: {
    ...
    omat4x4f_init_translate(&eye_shift, +(device->properties.ipd / 2.0f), 0.0f,
                            0.0f);
...
static int ohmd_device_setf_unp(ohmd_device *device, ohmd_float_value type,
                                const float *in) {
  switch (type) {
  case OHMD_EYE_IPD:
...
OHMD_APIENTRYDLL int OHMD_APIENTRY ohmd_device_geti(ohmd_device *device,
  ...
  case OHMD_CONTROLS_TYPES:
    memcpy(out, device->properties.controls_types,
           device->properties.control_count * sizeof(int));
    return OHMD_S_OK;

  case OHMD_CONTROLS_HINTS:
    memcpy(out, device->properties.controls_hints,
           device->properties.control_count * sizeof(int));
    return OHMD_S_OK;
...
static int ohmd_device_set_data_unp(ohmd_device *device, ohmd_data_value type,
                                    const void *in) {
  switch (type) {
...
OHMD_APIENTRYDLL ohmd_status OHMD_APIENTRY ohmd_device_settings_seti(
    ohmd_device_settings *settings, ohmd_int_settings key, const int *val) {
  switch (key) {
...
OHMD_APIENTRYDLL ohmd_device_settings *OHMD_APIENTRY
ohmd_device_settings_create(ohmd_context *ctx) {
  return ohmd_alloc(ctx, sizeof(ohmd_device_settings));
}

OHMD_APIENTRYDLL void OHMD_APIENTRY
ohmd_device_settings_destroy(ohmd_device_settings *settings) {
  free(settings);
}
...
void ohmd_calc_default_proj_matrices(ohmd_device_properties *props) {
  ...
  omat4x4f_init_perspective(&proj_base, props->fov, props->ratio, props->znear,
                            props->zfar);
...
void ohmd_set_universal_distortion_k(ohmd_device_properties *props, float a,
...
void ohmd_set_universal_aberration_k(ohmd_device_properties *props, float r,
                                     float g, float b) {
 ...
 */
uint64_t ohmd_monotonic_conv(uint64_t ticks, uint64_t srcTicksPerSecond,
                             uint64_t dstTicksPerSecond) {
