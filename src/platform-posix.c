...
void ohmd_monotonic_init(ohmd_context *ctx) {
  ...
  ctx->monotonic_ticks_per_sec =
      ts.tv_nsec >= 1000 ? NUM_1_000_000_000 : NUM_1_000_000_000 / ts.tv_nsec;
}
...
uint64_t ohmd_monotonic_get(ohmd_context *ctx) {
  ...
  return ohmd_monotonic_conv(now.tv_sec * NUM_1_000_000_000 + now.tv_nsec,
                             NUM_1_000_000_000, ctx->monotonic_ticks_per_sec);
}
...
ohmd_thread *ohmd_create_thread(ohmd_context *ctx,
                                unsigned int (*routine)(void *arg), void *arg) {
